-- Fix mutable search_path security warning on RPC functions.
-- Drop and recreate with schema-qualified table references and search_path = ''.

DROP FUNCTION IF EXISTS public.weekly_leaderboard(INTEGER);

CREATE FUNCTION public.weekly_leaderboard(result_limit INTEGER DEFAULT 100)
RETURNS TABLE (
  player_id UUID,
  x_handle TEXT,
  x_avatar TEXT,
...
AS $$
  ...
  auth_matches AS (
    ...
    FROM public.matches, week_bounds
  ...
  SELECT
    ...
    p.elo
  FROM agg
  JOIN public.players p ON p.id = agg.pid
  ORDER BY agg.wins DESC, agg.win_rate DESC
  LIMIT result_limit;
$$;

DROP FUNCTION IF EXISTS public.player_weekly_rank(TEXT);

CREATE FUNCTION public.player_weekly_rank(target_x_id TEXT)
RETURNS TABLE (
  "position" BIGINT,
  player_id UUID,
  x_handle TEXT,
...
AS $$
  ...
  auth_matches AS (
    ...
    FROM public.matches m, week_bounds
  ...
  SELECT
    ...
    p.elo
  FROM ranked r
  JOIN public.players p ON p.id = r.pid
